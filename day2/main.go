...
const input = `515,912,619,2043,96,93,2242,1385,2110,860,2255,621,1480,118,1230,99
...
func part_one() {
  ...
  for _, line := range lines {
    ...
    for _, num := range strings.Split(line,",") {
      normalized_num, _ := strconv.Atoi(num)
      if smallest == 0 {
        smallest = normalized_num
      }
      if normalized_num > largest {
        largest = normalized_num
      } else if normalized_num < smallest {
          smallest = normalized_num
      }
    }
